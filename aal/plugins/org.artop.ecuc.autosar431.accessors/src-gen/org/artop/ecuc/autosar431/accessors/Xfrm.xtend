...
class Xfrm implements IWrapper<GModuleConfiguration> {
	...
	def List<XfrmImplementationMapping> getXfrmImplementationMappings(){
		...
		return new BasicWrappingEList<XfrmImplementationMapping, GContainer>(filteredContainers, typeof(XfrmImplementationMapping), typeof(GContainer)) {
			override protected delegateAdd(org.artop.ecuc.autosar431.accessors.Xfrm.XfrmImplementationMapping xfrmImplementationMapping) {
			...
			override protected delegateAdd(int index, org.artop.ecuc.autosar431.accessors.Xfrm.XfrmImplementationMapping xfrmImplementationMapping) {
				xfrmImplementationMapping.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("XfrmImplementationMapping"))
	...
	static class XfrmImplementationMapping implements IWrapper<GContainer> {
		...
		static class XfrmDemEventParameterRefs implements IWrapper<GContainer> {
			...
			def org.artop.ecuc.autosar431.accessors.Dem.DemConfigSet.DemEventParameter getXFRM_E_MALFORMED_MESSAGE(){
				containerValue.getReference(typeof(org.artop.ecuc.autosar431.accessors.Dem.DemConfigSet.DemEventParameter), "XFRM_E_MALFORMED_MESSAGE")
			}
			
			def void setXFRM_E_MALFORMED_MESSAGE(org.artop.ecuc.autosar431.accessors.Dem.DemConfigSet.DemEventParameter object){
				val containerDef = containerValue.gGetDefinition
				if (containerDef instanceof GParamConfContainerDef) {
