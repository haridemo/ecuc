...
public class BooleanValueUnwrappingEList extends AbstractUnwrappingEList<EcucNumericalParamValue, Boolean> {
  private GConfigParameter parameterDef;
  
  public BooleanValueUnwrappingEList(final List<EcucNumericalParamValue> delegateList, final Class<EcucNumericalParamValue> wrapperType, final Class<Boolean> targetType, final GConfigParameter parameterDef) {
    super(delegateList, wrapperType, targetType);
    this.parameterDef = parameterDef;
  }
  
  protected EcucNumericalParamValue wrap(final Boolean object) throws CoreException {
  ...
  protected Boolean unwrap(final EcucNumericalParamValue parameterValue) {
    NumericalValueVariationPoint _value = parameterValue.getValue();
    String _mixedText = null;
    if (_value!=null) {
      _mixedText=_value.getMixedText();
    }
    return EcucValueAccessorUtil.toBoolean(_mixedText);
  }
}
