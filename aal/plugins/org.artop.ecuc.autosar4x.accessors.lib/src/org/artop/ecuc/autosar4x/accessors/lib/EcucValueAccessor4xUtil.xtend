...
class EcucValueAccessor4xUtil {
	...
	def static BigInteger getBigIntegerValue(GParameterValue parameterValue) {
		if (parameterValue instanceof EcucNumericalParamValue) {
			...
			return parameterValueValue?.mixedText.toBigInteger()
		}
	}
	...
	def static BigDecimal getBigDecimalValue(GParameterValue parameterValue) {
		if (parameterValue instanceof EcucNumericalParamValue) {
			val NumericalValueVariationPoint parameterValueValue = parameterValue.value
			parameterValueValue?.mixedText.toBigDecimal()
		}
	}
	...
	def static Boolean getBooleanValue(GParameterValue parameterValue) {
		if (parameterValue instanceof EcucNumericalParamValue) {
			val NumericalValueVariationPoint parameterValueValue = parameterValue.value
			parameterValueValue?.mixedText.toBoolean()
		}
	}
	...
	def static void setValue(GParameterValue parameterValue, Object parameterValueValue) {
		if (parameterValue instanceof EcucNumericalParamValue) {
			new GEcucNumericalParamValue40XAdapter(parameterValue).setValue(parameterValueValue?.toString)
	...
	def static GParameterValue createParameterValue(GConfigParameter parameterDefinition) {
		...
		var GParameterValue parameterValue = if (parameterDefinition instanceof EcucBooleanParamDef ||
			parameterDefinition instanceof EcucFloatParamDef || parameterDefinition instanceof EcucIntegerParamDef) {
			Autosar40Factory.eINSTANCE.createEcucNumericalParamValue
		} else if (parameterDefinition instanceof EcucAddInfoParamDef) {
			Autosar40Factory.eINSTANCE.createEcucAddInfoParamValue
		} else {
	...
	def static <T> void setReferenceValue(GContainer container, GConfigReference referenceDef,
		GContainer referenceValueValue) {
