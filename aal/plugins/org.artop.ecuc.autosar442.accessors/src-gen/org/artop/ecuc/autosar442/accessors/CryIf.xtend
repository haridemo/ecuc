...
class CryIf implements IWrapper<GModuleConfiguration> {
	...
	def List<CryIfChannel> getCryIfChannels(){
		...
		return new BasicWrappingEList<CryIfChannel, GContainer>(filteredContainers, typeof(CryIfChannel), typeof(GContainer)) {
			override protected delegateAdd(org.artop.ecuc.autosar442.accessors.CryIf.CryIfChannel cryIfChannel) {
				cryIfChannel.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("CryIfChannel"))
				super.delegateAdd(cryIfChannel)
			}

			override protected delegateAdd(int index, org.artop.ecuc.autosar442.accessors.CryIf.CryIfChannel cryIfChannel) {
				cryIfChannel.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("CryIfChannel"))
	...
	def List<CryIfKey> getCryIfKeys(){
		...
		return new BasicWrappingEList<CryIfKey, GContainer>(filteredContainers, typeof(CryIfKey), typeof(GContainer)) {
			override protected delegateAdd(org.artop.ecuc.autosar442.accessors.CryIf.CryIfKey cryIfKey) {
				cryIfKey.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("CryIfKey"))
				super.delegateAdd(cryIfKey)
			}

			override protected delegateAdd(int index, org.artop.ecuc.autosar442.accessors.CryIf.CryIfKey cryIfKey) {
				cryIfKey.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("CryIfKey"))
	...
	static class CryIfChannel implements IWrapper<GContainer> {
		...
		def org.artop.ecuc.autosar442.accessors.Crypto.CryptoDriverObjects.CryptoDriverObject getCryIfDriverObjectRef(){
			containerValue.getReference(typeof(org.artop.ecuc.autosar442.accessors.Crypto.CryptoDriverObjects.CryptoDriverObject), "CryIfDriverObjectRef")
		}
		
		def void setCryIfDriverObjectRef(org.artop.ecuc.autosar442.accessors.Crypto.CryptoDriverObjects.CryptoDriverObject object){
			val containerDef = containerValue.gGetDefinition
			if (containerDef instanceof GParamConfContainerDef) {
	...
	static class CryIfKey implements IWrapper<GContainer> {
		...
		def org.artop.ecuc.autosar442.accessors.Crypto.CryptoKeys.CryptoKey getCryIfKeyRef(){
			containerValue.getReference(typeof(org.artop.ecuc.autosar442.accessors.Crypto.CryptoKeys.CryptoKey), "CryIfKeyRef")
		}
		
		def void setCryIfKeyRef(org.artop.ecuc.autosar442.accessors.Crypto.CryptoKeys.CryptoKey object){
			val containerDef = containerValue.gGetDefinition
			if (containerDef instanceof GParamConfContainerDef) {
