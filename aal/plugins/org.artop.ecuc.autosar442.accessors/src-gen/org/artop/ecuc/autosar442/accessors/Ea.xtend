...
class Ea implements IWrapper<GModuleConfiguration> {
	...
	def List<EaBlockConfiguration> getEaBlockConfigurations(){
		...
		return new BasicWrappingEList<EaBlockConfiguration, GContainer>(filteredContainers, typeof(EaBlockConfiguration), typeof(GContainer)) {
			override protected delegateAdd(org.artop.ecuc.autosar442.accessors.Ea.EaBlockConfiguration eaBlockConfiguration) {
			...
			override protected delegateAdd(int index, org.artop.ecuc.autosar442.accessors.Ea.EaBlockConfiguration eaBlockConfiguration) {
				eaBlockConfiguration.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("EaBlockConfiguration"))
	...
	static class EaBlockConfiguration implements IWrapper<GContainer> {
		...
		def org.artop.ecuc.autosar442.accessors.Eep.EepGeneral getEaDeviceIndex(){
			containerValue.getReference(typeof(org.artop.ecuc.autosar442.accessors.Eep.EepGeneral), "EaDeviceIndex")
		}
		
		def void setEaDeviceIndex(org.artop.ecuc.autosar442.accessors.Eep.EepGeneral object){
			val containerDef = containerValue.gGetDefinition
			if (containerDef instanceof GParamConfContainerDef) {
