...
class WdgIf implements BasicWrappingEList.IWrapper<GModuleConfiguration> {
	...
	static class WdgIfDevice implements BasicWrappingEList.IWrapper<GContainer> {
		...
		def Integer getWdgIfDeviceIndex(){
			ParameterValueAccessor4xUtil.getIntegerValue(containerValue.gGetParameterValues.findFirst[gGetDefinition?.gGetShortName == "WdgIfDeviceIndex"])
		}
		...
		def void setWdgIfDeviceIndex(Integer value){
			...
			if (parameterValue == null) {
				...
				if (containerDef instanceof GParamConfContainerDef) {
					parameterValue = ParameterValueAccessor4xUtil.createParameterValue(containerDef.gGetParameters.findFirst[gGetShortName == "WdgIfDeviceIndex"])
	...
	static class WdgIfGeneral implements BasicWrappingEList.IWrapper<GContainer> {
		...
		def void setWdgIfDevErrorDetect(Boolean value){
			...
			if (parameterValue == null) {
				val containerDef = containerValue.gGetDefinition
				if (containerDef instanceof GParamConfContainerDef) {
					parameterValue = ParameterValueAccessor4xUtil.createParameterValue(containerDef.gGetParameters.findFirst[gGetShortName == "WdgIfDevErrorDetect"])
					containerValue.gGetParameterValues += parameterValue
				}
			}
			ParameterValueAccessor4xUtil.setParameterValue(parameterValue, value)
		...
		def void setWdgIfVersionInfoApi(Boolean value){
			...
			if (parameterValue == null) {
				val containerDef = containerValue.gGetDefinition
				if (containerDef instanceof GParamConfContainerDef) {
					parameterValue = ParameterValueAccessor4xUtil.createParameterValue(containerDef.gGetParameters.findFirst[gGetShortName == "WdgIfVersionInfoApi"])
					containerValue.gGetParameterValues += parameterValue
				}
			}
			ParameterValueAccessor4xUtil.setParameterValue(parameterValue, value)
