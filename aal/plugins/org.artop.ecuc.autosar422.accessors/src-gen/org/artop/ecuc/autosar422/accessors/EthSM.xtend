...
class EthSM implements IWrapper<GModuleConfiguration> {
	...
	def List<EthSMNetwork> getEthSMNetworks(){
		...
		return new BasicWrappingEList<EthSMNetwork, GContainer>(filteredContainers, typeof(EthSMNetwork), typeof(GContainer)) {
			override protected delegateAdd(org.artop.ecuc.autosar422.accessors.EthSM.EthSMNetwork ethSMNetwork) {
				ethSMNetwork.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("EthSMNetwork"))
				super.delegateAdd(ethSMNetwork)
			}

			override protected delegateAdd(int index, org.artop.ecuc.autosar422.accessors.EthSM.EthSMNetwork ethSMNetwork) {
				ethSMNetwork.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("EthSMNetwork"))
	...
	static class EthSMNetwork implements IWrapper<GContainer> {
		...
		def org.artop.ecuc.autosar422.accessors.ComM.ComMConfigSet.ComMChannel getEthSMComMNetworkHandleRef(){
			containerValue.getReference(typeof(org.artop.ecuc.autosar422.accessors.ComM.ComMConfigSet.ComMChannel), "EthSMComMNetworkHandleRef")
		}
		
		def void setEthSMComMNetworkHandleRef(org.artop.ecuc.autosar422.accessors.ComM.ComMConfigSet.ComMChannel object){
			val containerDef = containerValue.gGetDefinition
			if (containerDef instanceof GParamConfContainerDef) {
		...
		def org.artop.ecuc.autosar422.accessors.EthIf.EthIfConfigSet.EthIfController getEthSMEthIfControllerRef(){
			containerValue.getReference(typeof(org.artop.ecuc.autosar422.accessors.EthIf.EthIfConfigSet.EthIfController), "EthSMEthIfControllerRef")
		}
		
		def void setEthSMEthIfControllerRef(org.artop.ecuc.autosar422.accessors.EthIf.EthIfConfigSet.EthIfController object){
			val containerDef = containerValue.gGetDefinition
			if (containerDef instanceof GParamConfContainerDef) {
		...
		static class EthSMDemEventParameterRefs implements IWrapper<GContainer> {
			...
			def org.artop.ecuc.autosar422.accessors.Dem.DemConfigSet.DemEventParameter getETHSM_E_LINK_DOWN(){
				containerValue.getReference(typeof(org.artop.ecuc.autosar422.accessors.Dem.DemConfigSet.DemEventParameter), "ETHSM_E_LINK_DOWN")
			}
			
			def void setETHSM_E_LINK_DOWN(org.artop.ecuc.autosar422.accessors.Dem.DemConfigSet.DemEventParameter object){
				val containerDef = containerValue.gGetDefinition
				if (containerDef instanceof GParamConfContainerDef) {
