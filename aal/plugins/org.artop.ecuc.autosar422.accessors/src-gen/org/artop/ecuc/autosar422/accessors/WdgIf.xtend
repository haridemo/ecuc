...
class WdgIf implements IWrapper<GModuleConfiguration> {
	...
	def List<WdgIfDevice> getWdgIfDevices(){
		...
		return new BasicWrappingEList<WdgIfDevice, GContainer>(filteredContainers, typeof(WdgIfDevice), typeof(GContainer)) {
			override protected delegateAdd(org.artop.ecuc.autosar422.accessors.WdgIf.WdgIfDevice wdgIfDevice) {
				wdgIfDevice.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("WdgIfDevice"))
				super.delegateAdd(wdgIfDevice)
			}

			override protected delegateAdd(int index, org.artop.ecuc.autosar422.accessors.WdgIf.WdgIfDevice wdgIfDevice) {
				wdgIfDevice.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("WdgIfDevice"))
	...
	static class WdgIfDevice implements IWrapper<GContainer> {
		...
		def org.artop.ecuc.autosar422.accessors.Wdg.WdgGeneral getWdgIfDriverRef(){
			containerValue.getReference(typeof(org.artop.ecuc.autosar422.accessors.Wdg.WdgGeneral), "WdgIfDriverRef")
		}
		
		def void setWdgIfDriverRef(org.artop.ecuc.autosar422.accessors.Wdg.WdgGeneral object){
			val containerDef = containerValue.gGetDefinition
			if (containerDef instanceof GParamConfContainerDef) {
