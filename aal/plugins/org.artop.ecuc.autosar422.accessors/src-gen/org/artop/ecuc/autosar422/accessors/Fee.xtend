...
class Fee implements IWrapper<GModuleConfiguration> {
	...
	def List<FeeBlockConfiguration> getFeeBlockConfigurations(){
		...
		return new BasicWrappingEList<FeeBlockConfiguration, GContainer>(filteredContainers, typeof(FeeBlockConfiguration), typeof(GContainer)) {
			override protected delegateAdd(org.artop.ecuc.autosar422.accessors.Fee.FeeBlockConfiguration feeBlockConfiguration) {
			...
			override protected delegateAdd(int index, org.artop.ecuc.autosar422.accessors.Fee.FeeBlockConfiguration feeBlockConfiguration) {
				feeBlockConfiguration.target?.gSetDefinition(moduleConfiguration.getContainerDefinition("FeeBlockConfiguration"))
	...
	static class FeeBlockConfiguration implements IWrapper<GContainer> {
		...
		def org.artop.ecuc.autosar422.accessors.Fls.FlsGeneral getFeeDeviceIndex(){
			containerValue.getReference(typeof(org.artop.ecuc.autosar422.accessors.Fls.FlsGeneral), "FeeDeviceIndex")
		}
		
		def void setFeeDeviceIndex(org.artop.ecuc.autosar422.accessors.Fls.FlsGeneral object){
			val containerDef = containerValue.gGetDefinition
			if (containerDef instanceof GParamConfContainerDef) {
