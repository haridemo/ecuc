...
class V2xGn implements IWrapper<GModuleConfiguration> {
	...
	static class V2xGnGeneral implements IWrapper<GContainer> {
		...
		def org.artop.ecuc.autosar444.accessors.EthIf.EthIfConfigSet.EthIfController getV2xGnEthIfRef(){
			containerValue.getReference(typeof(org.artop.ecuc.autosar444.accessors.EthIf.EthIfConfigSet.EthIfController), "V2xGnEthIfRef")
		}
		
		def void setV2xGnEthIfRef(org.artop.ecuc.autosar444.accessors.EthIf.EthIfConfigSet.EthIfController object){
			val containerDef = containerValue.gGetDefinition
			if (containerDef instanceof GParamConfContainerDef) {
